...
class EmotivCortex2Client(WebsocketClient):
    ...
    def __init__(self,
                 ...
                 authenticate=False,
                 data_deque_size=10,
                 debug=False, license=None,debit=10):
...
''')
    ...
    def authorize(self):
        if self.authorized:
            return
        if self.license:
            params = {'clientId': self.client_id, 'clientSecret': self.client_secret, 'license': self.license,'debit':self.debit}
        else:
    ...
    def connect_headset(self, headset_id_idx=0, headset_id=None, mappings=None):
        ...
        self.headset_id=list(self.headsets)[headset_id_idx]
        if mappings is None:
    ...
    def update_headset(self, settings, headset_id_idx=0, headset_id=None):
        ...
        assert self.connected_headsets[headset_id][
                   'connectedBy'] == "usb cable", "Method supported only on wired connection only!"
    ...
    def create_record(self, title, session_id_idx=0, session_id=None, description=None, subject_name=None, tags=None):
        ...
        if tags is not None:
            params['tags'] = tags
        create_params=self.send_authed_request(method="createRecord", params=params)
        print(create_params)

        self.record_id=create_params['record']['uuid']
    ...
    def export_record(self, record_ids, folder, format, stream_types, version=None):
        ...
        for stream_type in stream_types:
            assert any([stream_type == val for val in
                        ["EEG", "MOTION", "PM", "BP"]]), "Valid stream types are: EEG, MOTION, PM, BP"
    ...
    def training(self, detection, status, action, session_id_idx=0, session_id=None):
        ...
        assert any([detection == val for val in
                    ["mentalCommand", "facialExpression"]]), "Valid detections are: mentalCommand or facialExpression"
    ...
    def facial_expression_signature_type(self, status, profile=None, session=None, signature=None):
        ...
        if signature is not None:
            assert any(
                [signature == val for val in ["universal", "trained"]]), "Valid signatures are: universal or trained"
    ...
    def mental_command_action_sensitivity(self, status, profile=None, session=None, values=[]):
        ...
        if status == "set":
            ...
            assert len(
                values) <= 4, "Only a maximum of 4 values can be passed in! Put them in order of the actions set in mental_command_active_action()"
